    ...
    <section className="rounded-xl border border-[#ebe7e3] bg-white p-5 shadow-sm dark:border-[#423238] dark:bg-[#2b2226]">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
        {actions.map(([id, Icon, label]) => (
          <button
            className="flex flex-col items-center gap-2 rounded-lg border border-[#ebe7e3] p-3 text-[11px] text-[#8b8383] transition hover:border-[#8f2039] hover:text-[#8f2039] dark:border-[#423238]"
            key={id}
            onClick={() => onAction(id)}
          >
            <Icon size={16} />
