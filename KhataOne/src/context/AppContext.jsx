...
export function AppProvider({ children }) {
  ...
  const customers = useMemo(
    () => data.customers.map((item) => customerStats(item, data.transactions)),
  ...
  const saveCustomer = (value) => {
    const exists = data.customers.some((item) => item.id === value.id);
    update({
      customers: exists
        ? updateCustomer(data.customers, value)
        : addCustomer(data.customers, value),
    });
    activity(exists ? "Customer updated" : "Customer added", value.name);
    notify("Customer saved");
  };
  const saveTxn = (value) => {
    const exists = data.transactions.some((item) => item.id === value.id);
    update({ transactions: saveTransaction(data.transactions, value) });
    activity(
      value.type === "payment"
        ? "Payment received"
        : exists
          ? "Transaction edited"
          : "Transaction created",
      money(value.amount, data.business.currency),
    );
  ...
  const deleteCustomer = (id) => {
    ...
    update({
      customers: removeCustomer(data.customers, id),
      transactions: data.transactions.filter(
        (transaction) => transaction.customerId !== id,
      ),
    });
    activity("Customer deleted", item?.name);
    notify("Customer deleted");
  };
  const deleteTxn = (id) => {
    update({ transactions: removeTransaction(data.transactions, id) });
  ...
  const reminder = (customer) => {
    ...
    activity("Reminder sent", `${customer.name} via WhatsApp`);
